...
function getTotalSpentByCategory(products) {
  let map = {};
  for (let i = 0; i < products.length; i++) {
    let product = products[i];
    if (!map[product.category]) {
      map[product.category] = product.price;
    } else {
      map[product.category] = map[product.category] + product.price;
    }
  }
...
getTotalSpentByCategory(userProducts);
