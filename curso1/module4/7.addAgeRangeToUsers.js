...
// Abaixo de 40 anos são jovens
// Acima de 60 anos são idosos
// Restante dizemos que estão na meia idade
function calculateUserAgeRange(user) {
  if (user.age < 40) {
    return "Young";
  } else if (user.age > 60) {
    return "Old";
...
function addAgeRangeToUsers(users) {
  for (let i = 0; i < users.length; i++) {
    let user = users[i];
    user.faixaEtaria = calculateUserAgeRange(user);
  }
}

addAgeRangeToUsers(users);
