...
function groupProductsByUserId(products) {
  let result = {};
  for (let i = 0; i < products.length; i++) {
    let product = products[i];
    if (!result[product.userId]) {
      result[product.userId] = [product];
    } else {
      result[product.userId].push(product);
    }
  }
...
groupProductsByUserId(userProducts);
